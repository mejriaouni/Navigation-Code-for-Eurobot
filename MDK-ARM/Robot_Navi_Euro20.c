...
//Speed Regulation
float speed_ref, ramp = 800,rampR=400,rampC=200;//max 1500
int sens;
double right_error=0,i_right_error=0;
double left_error=0,i_left_error=0;
float kp = 10.0 ,ki = 0.2;//0.2//10.0

//Move
int coef_correct_dist = 35;
...
void move_distance(float distance,float speed)
{
	...
	if (fabs(distance) < (speed*speed/ramp))
	{
		...
		speed = speed * accel_dist / ((float)0.5*speed*speed/ramp);
	}
	else
	{
	...
	while(fabs((total_right+total_left)/2-distance)>1)
	{
		...
		if (fabs((total_right+total_left)/2) < accel_dist)
			speed_ref = sens*50+sens*(speed-50)*fabs((total_right+total_left)/2)/(accel_dist);
	
		else if (fabs((total_right+total_left)/2 -distance) < decel_dist)
			speed_ref = sens*10+sens*(speed-10)*fabs((total_right+total_left)/2 -distance)/(decel_dist);
		else
...
while(fabs(total_centre)<fabs(goalC)){	
	t0=t;
		goalC=fabs(R*theta); 
	  remain_distC=goalC-fabs(total_centre);
...
void rotate(float angle, float speed)
{
	...
	if (fabs(goal_distance) < (2*speed*speed/rampR))
	{
		...
		speed = speed * accel_dist / ((float)speed*speed/rampR);
	}
	else
	{
	...
	//while( (fabs(total_right-total_left)-fabs(goal_distance)<-0.1) || (fabs(total_right-total_left)-fabs(goal_distance)>0.1) ){
	while( (fabs(total_right-total_left)<fabs(goal_distance)) ) {
		...
		if (fabs((total_right-total_left)) < accel_dist)
			speed_ref = sens*100+sens*(speed-100)*fabs(total_right-total_left)/(accel_dist);
		else if (fabs((total_right-total_left)-goal_distance) < decel_dist)
			speed_ref = sens*50+sens*(speed-50)*fabs((total_right-total_left) -goal_distance)/(decel_dist);
		else
			speed_ref = sens*speed;
		//Right wheel regulation
		right_error = speed_ref - right_speed;
